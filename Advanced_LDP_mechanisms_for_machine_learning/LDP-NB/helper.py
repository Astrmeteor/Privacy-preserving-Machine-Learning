...
def processData(dataMat, sessions, classifiers, system, privTgt='direction'):
    ...
    Xtrain, Xtest, ytrain, ytest = train_test_split(X, y, train_size=.8, stratify=y, random_state=randomState)
...
def getDirectionFlag(angle):
    ...
    if angle <= (0.25 * math.pi):
        return 4 # Right
    elif (0.25 * math.pi) < angle <= (1.25 * math.pi):
...
def getProjMat(X, y, type='NoDimRed', dimLen='def', subtype='1', priv='', rndSeed=0):
    ...
    if type == 'RND' and subtype == 1:
        ...
        rndProj = random_projection.GaussianRandomProjection(n_components=supRedDim, random_state=random_state)
        rndProj.fit(X)
...
if __name__ == "__main__":
    ...
    Xtrain, ytrain, Xlo, ylo, _ = prepareForTrainingWS(X, y, 2, cList, totalSize=20)
